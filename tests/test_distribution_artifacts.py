...
def test_demo_free_sdist_has_a_coherent_distributed_test_contract(tmp_path: Path) -> None:
    ...
    with tarfile.open(archive, "r:gz") as source:
        ...
        assert any(name.endswith("/packaging/macos/AudioAtlas.spec") for name in names)
        assert any(name.endswith("/scripts/build_macos_app.py") for name in names)
        assert any(name.endswith("/scripts/package_macos_dmg.py") for name in names)
        assert any(name.endswith("/scripts/build_windows_app.py") for name in names)
        assert any(name.endswith("/scripts/package_windows_candidate.py") for name in names)
        assert any(name.endswith("/packaging/windows/AudioAtlas.spec") for name in names)
        assert any(name.endswith("/packaging/windows/AudioAtlas.iss") for name in names)
        assert any(name.endswith("/docs/MACOS_APP_RELEASE.md") for name in names)
        assert any(name.endswith("/docs/MACOS_DEMO_GUIDE.md") for name in names)
        assert any(name.endswith("/docs/WINDOWS_DEMO_GUIDE.md") for name in names)

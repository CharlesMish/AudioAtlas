...
def _write_distributions(root: Path) -> dict[str, str]:
    (root / "audioatlas-0.2.0a8-py3-none-any.whl").write_bytes(b"wheel")
    (root / "audioatlas-0.2.0a8.tar.gz").write_bytes(b"source")
...
def test_distribution_hashes_requires_one_wheel_and_one_sdist(tmp_path: Path) -> None:
    ...
    assert expected == {
        "audioatlas-0.2.0a8-py3-none-any.whl": hashlib.sha256(b"wheel").hexdigest(),
        "audioatlas-0.2.0a8.tar.gz": hashlib.sha256(b"source").hexdigest(),
    }
...
def test_compare_release_distinguishes_absent_and_exact(tmp_path: Path) -> None:
    ...
    assert compare_release(None, version="0.2.0a8", expected=expected) == "absent"
    assert compare_release({"releases": {}}, version="0.2.0a8", expected=expected) == "absent"
    assert (
        compare_release(_payload("0.2.0a8", expected), version="0.2.0a8", expected=expected)
        == "exact"
    )

...
def plot_average_spectrum(
    ...
    *,
    ...
    mask = freqs >= 20
    ax.semilogx(freqs[mask], spectrum.power_db[mask], linewidth=1.1, **plot_role("s1"))
    ax.set_title(title)
    ...
    for freq, label in [
        ...
        if 20 <= freq <= freqs[-1]:
            ax.axvline(freq, linestyle="--", linewidth=0.6, **plot_role("grid", alpha=0.45, essential=False))
            ax.text(
                freq, ax.get_ylim()[1], label,
                rotation=90, va="top", ha="right", fontsize=8, **plot_role("labels"),
            )
